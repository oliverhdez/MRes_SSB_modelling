...
def sdot_mother(s,t,params):
	...
	kdil, kconstitutive, kinflux, km, kcat, kdegrad, kinduced, permeability_IAAH, permeability_IAA, fIAAH_w, fIAAH_c, Nµ  = params
	...
	dAUXIN = ((IAAH*kcat*IAM)/(km + IAM)) - AUXIN*kdil - permeability_IAAH*(fIAAH_c*AUXIN-fIAAH_w*0) - 10000*permeability_IAA*PIN2*Nµ*(1-fIAAH_c)*AUXIN # Assume auxin disperses too fast therefore no influx back into the cell. If looking at emergent properties, simply rearrange the efflux equation into influx
...
#	- That means on average 0.378 µmol/L of each protein per gene
	...
	kinflux = 0.00356/((4.4*(9**(-14)))*1.38) # Based on the target steady state of 10µM of IAM inside cells. Denominator is the average surface area of a haploid yeast cell.
	...
	kinduced = 0.000743 #based on a violacein target steady state of 1µM
	...
	params = kdil, kconstitutive, kinflux, km, kcat, kdegrad, kinduced,  permeability_IAAH, permeability_IAA, fIAAH_w, fIAAH_c, Nµ
